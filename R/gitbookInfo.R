...
gitbookInfo <- function() {
	...
	if(length(current) > 0) {
		...
		if(current == installed) {
			message(paste0('gitbook-cli is up-to-date with version ', current))
		} else {
			message(paste0('A new version of gitbook-cli is available. Version ',
						   installed, ' installed, ', current, ' available.'))
		}
	} else {
		warning(paste0('Could not get the current available version of ',
